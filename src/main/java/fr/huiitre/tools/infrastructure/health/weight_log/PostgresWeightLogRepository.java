package fr.huiitre.tools.infrastructure.health.weight_log;
...
public class PostgresWeightLogRepository implements WeightLogRepository {
    ...
    private static final RowMapper<WeightLog> WEIGHT_LOG_ROW_MAPPER = (rs, rowNum) ->
        new WeightLog(
            rs.getDouble("weight"),
    ...
    private static final RowMapper<WeightLogView> WEIGHT_LOG_VIEW_ROW_MAPPER = (rs, rowNum) ->
        new WeightLogView(
            rs.getLong("id"),
            rs.getDouble("weight"),
            rs.getTimestamp("logged_at").toLocalDateTime(),
